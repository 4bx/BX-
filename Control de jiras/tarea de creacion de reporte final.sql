select c_status, paso, key_, issue_type, status, 
		...
		concat(date_format(fecha_inicio, '%Y/%m/%d '),time_format(fecha_inicio,'%H:%i:%S'))as fecha_inicio, 
	...
	concat(date_format(date_add(fecha_inicio ,interval case when nivel_servicio = "Enhancement" then 5 else 1 end day ) , '%Y/%m/%d '),time_format(date_add(fecha_inicio ,interval case when nivel_servicio = "Enhancement" then 5 else 1 end day ),'%H:%i:%S')) as fecha_prometida__vs_ns, 
